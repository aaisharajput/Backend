        ...
        get_to_do(function(todos){
            ...
            todos.forEach(function(todo){
                ...
                <img src="${todo.pics}" id="img${key_no}">
                ...
                <input type='checkbox' onclick='complete(${key_no})' class='form-check-input position-relative float-right mr-5' id='${key_no}' ${(todo.checked==true)?"checked":'' }></div>`;                    
        ...
        function press_enter(event){
            ...
            if(event.keyCode==13){
                ...
                if(data!='' && pics.value!="")
                {
                    ...
                    setTimeout(function(){
                        let imgg=document.getElementById("img"+i);
                        let obj=new FileReader();
                        obj.onload=function(){
                            imgg.src=obj.result;
                        }
    
                        obj.readAsDataURL(pics.files[0]);
                    },100);
        ...
        function push_to_do(i,data,callback){
            let request = new XMLHttpRequest();
                ...
                // let task={id:i,task_list:data,checked:false,pics:pics.files[0]};
                let formData=new FormData();
                formData.append("id",i);
                formData.append("task_list",data);
                formData.append("checked",false);
                formData.append("pics",pics.files[0]);
