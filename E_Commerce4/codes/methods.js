...
function loadMoreProduct(req,res,next){
    read_file_product(res,function(details){
        ...
        if(Object.keys(details).length>=indx){
            ...
            // k.forEach(function(i,j){
            //     if(j>=indx-5)
            //         p_data.push(details[i]);
            //     if(j==indx-1)
            //         return;
            // })
...
function loadAdminProduct(req,res){
    read_file_product(res,function(details){
        res.json(details);
    })
}
...
function deleteAdminProduct(req,res,next){
    const p_id=req.body.id;
    read_file_product(res,(Data)=>{
        delete Data[p_id];
       write_file_product(Data,(err)=>{
        if(err){
            next();
            return;
        }
...
function addAdminProduct(req,res,next){
    read_file_product(res,(data)=>{
        let lastkey=Date.now();
        let prodt=req.body;
        lastkey++;
        prodt.img=req.file.filename;
        prodt["id"]=lastkey;
        
        data[lastkey]=prodt;
        write_file_product(data,(err)=>{
            if(err){
                res.render("admin",{user:req.session.user.user,msg:"Error ocured!!"+err});
            }
            next();
            return;
        })
...
function updateAdminProduct(req,res,next){
    ...
    if(p_data.img==undefined)
        p_data["img"]=req.file.filename;
     read_file_product(res,(data)=>{
        data[p_data.id]=p_data;
        write_file_product(data,(err)=>{
            if(err){
...
function write_file_product(data,callback){
    fs.writeFile("./product_details.txt",JSON.stringify(data),callback);
}
...
module.exports={chkAuthenticationLogin,addAdminProduct,deleteAdminProduct,updateAdminProduct,chkAdmin,chkAuthenticationIndex,chkAuthenticationSignup,loadAdminProduct,plusMinusProduct,deleteProduct,redirectLogin,singleProduct,cardItems,addProductToCart,loadMoreProduct,sign_up,send_mail,verify_token,log_in,changePswd,forgotPswd,verify_email,changePswdforgot};